...
QBCore.Commands.Add("logout", "Logout of Character (Admin Only)", {}, false, function(source)
    ...
    TriggerClientEvent('mrf_charmenu:client:chooseChar', src)
end, "mod")

QBCore.Commands.Add("closeNUI", "Close Multi NUI", {}, false, function(source)
    local src = source
    TriggerClientEvent('mrf_charmenu:client:closeNUI', src)
end)

-- Events

RegisterNetEvent('mrf_charmenu:server:disconnect', function()
    local src = source
    DropPlayer(src, "You have disconnected from QBCore")
end)

RegisterNetEvent('mrf_charmenu:server:loadUserData', function(cData)
    local src = source
...
RegisterNetEvent('mrf_charmenu:server:createCharacter', function(data)
    local src = source
    local newData = {}
    ...
    if QBCore.Player.Login(src, false, newData) then
        if Config.StartingApartment then
            ...
            QBCore.Commands.Refresh(src)
            loadHouseData()
            TriggerClientEvent("mrf_charmenu:client:closeNUIdefault", src)
            GiveStarterItems(src)
        end
	end
end)

RegisterNetEvent('mrf_charmenu:server:deleteCharacter', function(citizenid)
    local src = source
...
QBCore.Functions.CreateCallback("mrf_charmenu:server:GetUserCharacters", function(source, cb)
    local src = source
...
QBCore.Functions.CreateCallback("mrf_charmenu:server:GetServerLogs", function(source, cb)
    MySQL.Async.execute('SELECT * FROM server_logs', {}, function(result)
        cb(result)
    end)
end)

QBCore.Functions.CreateCallback("mrf_charmenu:server:GetNumberOfCharacters", function(source, cb)
...
QBCore.Functions.CreateCallback("mrf_charmenu:server:setupCharacters", function(source, cb)
    local license = QBCore.Functions.GetIdentifier(source, 'license')
    local plyChars = {}
...
QBCore.Functions.CreateCallback("mrf_charmenu:server:getSkin", function(_, cb, cid)
...
print("^2Charmenu ^7- ^2Made by ^1MARFY^7")